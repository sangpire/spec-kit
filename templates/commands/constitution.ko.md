...
`/memory/constitution.md`에서 프로젝트 헌장을 업데이트하고 있습니다. 이 파일은 대괄호 안에 플레이스홀더 토큰이 포함된 템플릿입니다 (예: `[PROJECT_NAME]`, `[PRINCIPLE_1_NAME]`). 귀하의 작업은 (a) 구체적인 값을 수집/도출하고, (b) 템플릿을 정확하게 채우고, (c) 의존 결과물에 개정 사항을 전파하는 것입니다.
...
4. 일관성 전파 체크리스트 (이전 체크리스트를 활성 검증으로 변환):
   ...
   - `/templates/commands/*.md`의 각 명령 파일 읽기 및 일반 가이드가 필요할 때 오래된 참조가 남아있지 않은지 확인
